# MeshDrop · linux-tui 端 Backend 接入完整 Prompt（自动拼接）

> 这份 prompt 由 prompts/feed.sh 拼接生成：
>   B-COMMON.md + ../protocol/companion-bridges.md + B06-linux-tui.md + B-TESTING.md
> 复制整段给 AI，它将拥有把 linux-tui 端 UI 接入真实 backend 的全部信息。

...
pub fn build(handle: Option<&Rc<AppHandle>>) -> gtk::Widget {
    ...
    let title = gtk::Label::new(Some(&*t!("history.title")));
    ...
    sp.set_hexpand(true);
    title_row.append(&sp);
    let count_chip = chip::chip(&t!("history.chip_all", count = 0), chip::Tone::Ink, true);
    title_row.append(&count_chip);
    title_row.append(&chip::chip(&t!("history.chip_files"), chip::Tone::Outline, true));
    title_row.append(&chip::chip(&t!("history.chip_text"), chip::Tone::Outline, true));
    title_row.append(&chip::chip(&t!("history.chip_image"), chip::Tone::Outline, true));
    // 清空历史按钮 —— 仅真实模式生效，mock（screenshots）下禁用。
    let clear_btn = icon_btn::icon_btn(&t!("history.clear_btn"), &t!("history.clear_btn_tip"), icon_btn::IconBtnTone::Danger);
    ...
    let today_div = ascii_divider::build(&t!("history.today_divider", count = 0));
    ...
    today_div.set_text(&t!("history.today_divider", count = initial.len()));
    count_chip.set_tooltip_text(Some(&*t!("history.total_count", count = initial.len())));
    ...
    if let Some(h) = handle {
        ...
        h.observe(h.engine.history_rx(), move |items| {
            ...
            today_lbl.set_text(&t!("history.today_divider", count = views.len()));
        });
    }
...
fn history_card(item: &ViewHistoryRow) -> gtk::Box {
    ...
    let arrow_chip = match item.dir {
        mock::Dir::In  => chip::chip(&t!("history.in"), chip::Tone::Sky,   true),
        mock::Dir::Out => chip::chip(&t!("history.out"), chip::Tone::Flame, true),
    };
    ...
    let time = gtk::Label::new(Some(&*t!("history.time_today", time = item.time)));
    time.add_css_class("meshdrop-meta");
    time.set_halign(gtk::Align::End);
    meta_col.append(&time);

    let (status_key, status_tone) = match item.status {
        mock::HistoryStatus::Done         => ("history.status_done", chip::Tone::Lime),
        mock::HistoryStatus::Transferring => ("history.status_transferring", chip::Tone::Flame),
        mock::HistoryStatus::Queued       => ("history.status_queued", chip::Tone::Outline),
        mock::HistoryStatus::Failed       => ("history.status_failed", chip::Tone::Error),
    };
    let status_chip = chip::chip(&t!(status_key), status_tone, true);
...
fn build_empty_card() -> gtk::Box {
    ...
    card.add_css_class("meshdrop-card");
    let title = gtk::Label::new(Some(&*t!("history.empty_title")));
    title.add_css_class("meshdrop-card-title");
    title.set_halign(gtk::Align::Start);
    card.append(&title);
    let hint = gtk::Label::new(Some(&*t!("history.empty_hint")));
...
fn view_from_mock(m: &mock::HistoryRow) -> ViewHistoryRow {
    let kind = match &m.kind {
        ...
        mock::HistoryKind::Image { count } => ViewHistoryKind::Text(t!("history.images_count", count = count).to_string()),
    };
    ViewHistoryRow {
        id: m.id.to_string(),
        dir: m.dir,
